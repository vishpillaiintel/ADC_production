...
TET_OPERS = []
TET_XML_SOURCE_PATH = r"C:\HEX_ADC\TET\MAU_INPUT\XML\source" 
TET_XML_PROCESSING_PATH = r'C:\HEX_ADC\TET\XML\processing'
TET_XML_PROCESSED_PATH = r'C:\HEX_ADC\TET\XML\processed'
TET_LOG_FILE = r'C:\HEX_ADC\TET\Log\LogFile.txt'
TET_ERROR_LOG_FILE = r'C:\HEX_ADC\TET\Log\ErrorLog.txt'
TET_CSV_ARCHIVE = r'C:\HEX_ADC\TET\archive'
TET_CSV_PROCESSED = r'C:\HEX_ADC\TET\processed'
TET_CSV_PROCESSING = r'C:\HEX_ADC\TET\processing'
...
def setup_logging():
    ...
    handler = logging.handlers.RotatingFileHandler(TET_LOG_FILE, mode='a', maxBytes=10485760, backupCount=10)
    handler.setLevel(logging.INFO)
    
    error_handler = logging.handlers.RotatingFileHandler(TET_ERROR_LOG_FILE, mode='a', maxBytes=10485760, backupCount=10)
...
def create_temp(panel_csv, resize=1):
    ...
    temp_panel = os.path.join(TET_PANEL_TEMP, panel_csv.split('_Defect_List')[0])
...
def run_ADC_processing(xml_file):
    ...
    if operation_run not in TET_OPERS:
    ...
    move(xml_file, TET_XML_PROCESSING_PATH)
    ...
    for panel_file in doc_in_list:
        ...
        shutil.copy(panel_file,TET_CSV_PROCESSING)
        ...
        model = hex_model(model_path=os.listdir(os.path.join('models','TET'))[0], output_folder=t_output, \
        ...
        dff_output_path = os.path.join(TET_CSV_OUTPUT, os.path.basename(panel_file))
    ...
    for processed_files in os.listdir(TET_CSV_PROCESSING):
        move(TET_CSV_PROCESSING + "\\" + processed_files, TET_CSV_PROCESSED)

    logger.info('Moving ADC output files to archive')
    for output_file in os.listdir(TET_CSV_OUTPUT):
        move(output_file, TET_CSV_ARCHIVE)

    logger.info('Moving XML to processed folder')
    move(xml_file, TET_XML_PROCESSED_PATH)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        lot_list = get_xml_simple(TET_XML_SOURCE_PATH)
